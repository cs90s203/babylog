...
function renderFeedStats(state) {
  ...
  const amtChart = sCard('奶量 ml（母乳 ＋ 配方）',
    `<div style="display:flex;align-items:flex-end;gap:6px;height:120px;">${totals.map((tot, i) => `<div onclick="A.toggleMlBar(${i})" style="flex:1;display:flex;flex-direction:column;align-items:center;gap:6px;cursor:pointer;"><div style="font-size:9px;color:var(--text3);font-weight:700;">${tot}</div><div style="width:100%;height:${Math.round(tot / aMax * 64)}px;border-radius:6px;overflow:hidden;display:flex;flex-direction:column;outline:${expandedIdx === i ? '2px solid var(--accent)' : 'none'};outline-offset:2px;">${tot > 0 ? `<div style="height:${formulaMl[i] / tot * 100}%;background:#E8A33D;"></div><div style="flex:1;background:#FF8C6B;"></div>` : ''}</div><div style="font-size:9px;color:var(--text2);font-weight:600;">${labels[i]}</div>${dateSubLabel(i)}</div>`).join('')}</div>
...
function renderGrowthSummaryCard() {
  ...
  if (growth.length >= 2) {
    ...
    // "上個月到這個月" — compare against whichever earlier record sits closest to (at or
    // before) one month before the latest, so this reads as an actual month of growth rather
    // than the very first record ever logged. Falls back to the oldest available record if
    // nothing is that old yet (e.g. tracking only started a couple weeks ago).
    const prior = growth.slice(0, -1);
    const atLeastAMonthOld = prior.filter((g) => new Date(g.date) <= monthAgo);
    const baseline = atLeastAMonthOld.length ? atLeastAMonthOld[atLeastAMonthOld.length - 1] : prior[0];
    const deltas = [['weight', '⚖️', 'kg'], ['height', '📏', 'cm']]
      .filter(([k]) => baseline[k] != null && latest[k] != null)
      .map(([k, emoji, unit]) => `${emoji} ${(latest[k] - baseline[k] >= 0 ? '+' : '')}${(latest[k] - baseline[k]).toFixed(1)}${unit}`);
    if (deltas.length) {
      deltaLine = `<p style="font-size:12.5px;color:var(--text2);margin-top:6px;">${esc(baseline.date)} 到 ${esc(latest.date)}：${deltas.join('　')}</p>`;
    }
  }
  return `<div class="card" style="padding:16px;margin-bottom:14px;">${ageLine}${deltaLine}</div>`;
}

// Curated infant/toddler development reference points, one entry per month (0–24), plus 30
// and 36 as the next real checkpoints past 2 years — NOT sourced live, but researched from
// CDC's "Learn the Signs. Act Early." checklists, AAP Bright Futures periodicity guidance,
// and WHO complementary-feeding guidance (see the discussion that added this, 2026-08).
// Deliberately worded soft ("多數"/"開始"/"約") because every baby's timeline varies; this is
// a general reference, not a checklist to hit on schedule. Ages between 24 and 36 months fall
// back to the nearest PRECEDING checkpoint (see findMilestoneData) rather than getting
// fabricated month-by-month content — real pediatric references don't get more granular than
// 24/30/36 months either, so this matches how the source material is actually organized.
const GROWTH_MILESTONES = [
  { month: 0, tips: [
    '多具備覓食、吸吮等反射動作，聞到味道會轉頭尋找乳房',
    '受驚或聲響較大時可能出現驚跳反射，雙手張開後收回，屬正常現象',
    '多數時間都在睡覺，一天約睡16小時，日夜作息尚未穩定',
    '約每2–3小時哺餵一次，一天約8–12次，依飢餓訊號調整',
    '視力仍模糊，多半只能看清20–30公分內的高對比臉孔',
    '出生後3–5天內，多建議安排第一次新生兒健康檢查',
  ], watch: [
    '每次睡眠都讓寶寶仰睡，選用堅實平坦床墊，避免傾斜臥具',
    '嬰兒床內盡量淨空，不放枕頭、被子、床圍與玩偶',
    '建議與寶寶同室不同床，至少維持到6個月大',
    '汽座、提籃等非平坦臥具不適合作為日常睡眠場所',
    '避免劇烈搖晃寶寶，訪客接觸寶寶前提醒先洗手',
  ]},
  { month: 1, tips: [
    '趴姿時能短暫抬頭、左右轉動，四肢仍以反射動作為主',
    '雙手多呈握拳狀，開始能將手帶到眼睛或嘴巴附近',
    '視線可聚焦約20–30公分，喜歡看人臉與高對比圖案',
    '聽力發育已較成熟，對熟悉的聲音或說話聲會有反應',
    '對甜味較有偏好，也能辨認熟悉照顧者的氣味',
    '清醒時間漸漸拉長，仍以睡眠為主要活動',
  ], watch: [
    '趴著玩耍需全程有人在旁看顧，避免不慎趴睡',
    '嬰兒床上方勿於睡眠時段懸掛固定吊飾，玩具睡前收好',
    '避免包覆過厚或環境過熱，留意體溫過高的徵兆',
    '家中訪客接觸寶寶前，提醒先洗手降低感染風險',
    '持續維持仰睡、同室不同床等安全睡眠原則',
  ]},
  { month: 2, tips: [
    '開始對人微笑，會試著注視照顧者的臉孔',
    '發出咕咕等喉音，逗弄時會有聲音回應',
    '頭會轉向聲音來源，開始用眼睛追視移動物品',
    '趴姿時能將頭稍微抬離床面，手腳動作更流暢',
    '情緒表達更豐富，會用吸手等方式安撫自己',
    '若活動一成不變，較容易出現哭鬧、不耐煩反應',
  ], watch: [
    '2個月起常安排疫苗接種，接種後如有發燒等異常請洽醫師',
    '抬頭力氣增加，趴姿玩耍仍需全程在旁看顧',
    '尚無法自行翻身，但動作漸增，勿獨留寶寶於高處',
    '持續仰睡與嬰兒床淨空原則，不放軟墊玩偶',
    '抱哭鬧的寶寶動作放輕，避免劇烈搖晃身體',
  ]},
  { month: 3, tips: [
    '趴姿時能用前臂撐起上半身，抬頭角度更高更穩',
    '雙手會張開，主動揮動或拍打眼前的玩具',
    '微笑更具社會性，常主動用笑容、咿呀聲吸引互動',
    '情緒表達更明顯，喜歡「一來一往」的逗弄互動',
    '多建議純哺餵母乳至6個月，配方奶則依需求哺餵',
    '日夜作息漸趨規律，夜間睡眠時間慢慢拉長',
  ], watch: [
    '部分寶寶已能翻身，勿獨留寶寶於床、沙發等高處',
    '換尿布、洗澡台等高處務必隨時扶著，不轉身離開',
    '玩具避免細小零件，留意誤吞誤塞風險',
    '未滿4–6個月，多不建議提早添加副食品',
    '持續仰睡與嬰兒床淨空原則，留意環境不過熱',
  ]},
  { month: 4, tips: [
    '開始會自發性微笑，特別是看到熟悉的臉孔時',
    '趴姿時能用手肘撐起上半身，頭部也逐漸穩定',
    '開始牙牙學語，發出「啊」「喔」等聲音並模仿語調',
    '手眼協調進步，會伸手抓取眼前的玩具',
    '肚子餓時看到奶瓶或準備哺乳會張嘴期待',
    '夜間睡眠可能開始拉長，部分寶寶逐漸減少半夜餵奶次數',
  ], watch: [
    '一旦出現翻身跡象就該停止包巾睡眠，避免趴睡窒息風險',
    '趴姿玩耍務必全程在旁看顧，避免口鼻遭棉被等物品悶住',
    '口腔期發展明顯，小物品、繩帶類要收好避免誤吞',
    '可提前檢視居家環境（插座、家具邊角），為活動力增加做準備',
  ]},
  { month: 5, tips: [
    '翻身動作更純熟，前後翻身逐漸穩定進步中',
    '對聲音來源更敏銳，會轉頭尋找聲音方向',
    '開始能坐得較穩，但多數仍需要旁人或靠墊支撐',
    '看大人用餐時可能開始表現出興趣與好奇',
    '抓握更有目的性，會主動抓玩具並放入口中探索',
    '睡眠模式因人而異，多數逐漸朝較長的夜間睡眠發展',
  ], watch: [
    '一旦開始翻身，尿布台等高處絕對不能讓寶寶單獨停留',
    'WHO建議約6個月再開始添加副食品，不需提前',
    '寶寶愛把東西放入口中，小型物品、電池、硬幣要收好',
    '坐姿尚不穩定時務必在旁扶持，避免跌倒撞到頭部',
  ]},
  { month: 6, tips: [
    '前後翻身已相當熟練，開始能不需支撐坐一會兒',
    '站立時能用腿部撐體重，有時會開心地彈跳',
    '開始發出子音牙牙學語（如「ㄇ」「ㄅ」），並會回應自己的名字',
    '喜歡照鏡子，看到鏡中自己會感興趣並微笑',
    '世界衛生組織建議此階段可開始嘗試副食品，同時持續哺乳',
    '會用雙手交替傳遞物品，對搆不到的東西感到好奇',
  ], watch: [
    '會翻身坐起後，尿布台、床鋪等高處都不可讓寶寶單獨停留',
    '添加副食品初期質地需細滑，一次只加一種新食物觀察反應',
    '避免蜂蜜、整顆堅果、葡萄等高風險食物，降低嗆咳風險',
    '坐姿玩耍時周圍避免尖角家具，隨時留意跌倒',
    '開始加裝插座蓋、櫃門鎖等居家防護，因活動力持續增加',
  ]},
  { month: 7, tips: [
    '坐姿更穩定，逐漸不需扶持也能坐好一段時間',
    '開始出現以肚子為軸心挪動或後退的動作，是爬行前的過渡',
    '副食品質地可從細泥逐漸進展到較濃稠或小塊狀',
    '對躲貓貓等重複性遊戲展現興趣，開始理解簡單因果關係',
    '可能開始出現怕生反應，對主要照顧者更加依附',
    '咀嚼吞嚥能力持續發展，能嘗試較多樣的食物質地',
  ], watch: [
    '坐姿及爬行前活動增加，插座、家具固定等居家防護要落實',
    '寶寶已能自行挪動身體，避免讓他單獨留在床或沙發上',
    '食物質地增加時仍要切小塊，避免整顆堅果與圓形硬食物',
    '出現怕生反應時給予耐心安撫，不強迫與陌生人互動',
    '低處櫃子、垃圾桶等要收納或加裝安全鎖，因活動範圍擴大',
  ]},
  { month: 8, tips: [
    '多數寶寶能坐得比較穩，雙手漸漸能空出來玩玩具',
    '手指開始出現不成熟的鉗狀抓握，能撿起較小物品',
    '有些寶寶開始嘗試爬行，移動方式因人而異都算正常',
    '副食品質地可從糊狀慢慢進階到較軟的手指食物',
    '開始認得熟悉臉孔，對物品「不見了」會有反應',
    '依戀主要照顧者的傾向漸增，夜間可能睡得較不安穩',
  ], watch: [
    '寶寶會坐之後建議把嬰兒床調到最低，預防日後扶站跌出',
    '開始練習爬行前，先收好地上小物、電線與尖銳物品',
    '副食品換質地時仍要確認軟硬與大小，降低噎嗆風險',
    '吃東西時讓寶寶坐好，避免邊爬邊玩邊吃',
    '硬幣、鈕扣電池等小物要收到完全拿不到的地方',
  ]},
  { month: 9, tips: [
    '多數寶寶能自己坐穩，也開始扶著家具站起來',
    '開始用爬行或其他方式在家中到處移動探索',
    '會用拇指和食指撿起小東西，雙手能左右換手玩玩具',
    '聽得懂「不可以」，會發出「媽媽媽媽」「爸爸爸爸」疊音',
    '喜歡玩躲貓貓，也會模仿大人的聲音和動作',
    '開始認生，對熟悉的照顧者變得比較黏',
  ], watch: [
    '寶寶開始扶站爬行，樓梯口建議加裝安全柵欄',
    '能穿過衛生紙捲筒的小物都有噎嗆風險，務必收好',
    '吃東西務必坐在餐椅上，不要邊爬邊走邊吃',
    '認生、分離焦慮可能讓寶寶夜醒變多，屬正常現象',
    '電源插座、電線、藥品清潔劑要收好或加裝防護',
  ]},
  { month: 10, tips: [
    '開始扶著家具「巡航」移動，從扶站進階到扶走',
    '鉗狀抓握更精準，能撿起豆子、米果等小東西',
    '副食品可進階到剁碎切小塊，鼓勵自己動手抓食',
    '開始聽懂自己的名字，被叫名字時會有反應',
    '有些寶寶放開手扶物，能短暫站得住幾秒鐘',
    '白天多維持兩次小睡，睡眠總時數約12到16小時',
  ], watch: [
    '寶寶扶物走動時，矮櫃、電視要固定避免傾倒',
    '手抓食物、豆類要確認軟硬與大小，降低噎到風險',
    '樓梯口、陽台等高低落差處要加裝安全門或圍欄',
    '藥品清潔劑要收到寶寶爬不到扶不到的高處並上鎖',
  ]},
  { month: 11, tips: [
    '放開手扶物嘗試站立，有些寶寶會放手跨出一兩步',
    '扶著東西能蹲下撿東西，再自己站起來',
    '開始有意義地說出「媽媽」「爸爸」等疊音詞',
    '會用簡單手勢表達，像搖頭或揮手說掰掰',
    '開始練習用杯子喝水，也更愛自己動手抓東西吃',
    '喜歡把東西放進容器再倒出來，重複玩同個遊戲',
  ], watch: [
    '寶寶隨時可能放手站立跨步，桌角建議加裝防撞護角',
    '練習用杯子喝水時在旁陪同，避免嗆到',
    '窗簾百葉窗拉繩要收高固定，避免纏繞寶寶',
    '寶寶站立能構到的高度變高，熱飲小物要移到更高處',
  ]},
  { month: 12, tips: [
    '會玩「躲貓貓」「拍手歌」等互動遊戲',
    '多數會用「媽媽」「爸爸」或特定稱呼叫人',
    ...
    '開始會把積木等小物放進容器裡',
    '扶著家具站立、扶走，手指精細動作進步',
    '飲食可安排一天3–4次正餐、1–2次點心',
  ], watch: [
    '家具要固定好，避免扶站時被拉倒',
    '小塊食物、小物品是常見哽塞風險，需切碎剪小',
    '藥品、清潔劑等要上鎖收好，遠離寶寶',
    '不建議使用學步車，以免影響安全與發展',
    '樓梯、插座等要加裝防護，因應扶走行動力增加',
  ]},
  { month: 13, tips: [
    '扶走更加穩定，有些寶寶開始嘗試放手站立',
    '少數寶寶會跨出第一步，但時間差異很大',
  ...
  { month: 14, tips: [
    ...
    '會嘗試把兩個小物疊起來或組合玩玩具',
    '對熟悉的人會更主動表現喜歡、想親近',
    '用手指自行進食更穩定，願意嘗試新口感食物',
  ], watch: [
    '若已能放手走幾步，居家防撞防滑要再加強',
    '開始出現情緒起伏或短暫發脾氣，屬常見現象',
    '樓梯口、陽台等高低落差處要做好防護',
    '藥品、清潔用品持續上鎖，誤食風險仍在',
  ]},
  { month: 15, tips: [
    ...
    '剛學走路易跌倒，居家防撞防滑要留意',
    '情緒爆發（tantrum）常見，環境仍要顧及安全',
    '2歲前建議不安排螢幕時間（視訊通話除外）',
    '哽塞食物風險持續存在，避免整顆葡萄、堅果等',
    '每天建議睡眠11–14小時，含午睡，作息盡量固定',
  ]},
  { month: 16, tips: [
    '多數寶寶已能放手獨走，開始嘗試扶著東西邁出小步伐',
    '開始模仿大人的簡單動作，例如假裝打電話、梳頭髮',
    '詞彙量緩慢增加，常見會說1–2個「媽媽爸爸」以外的字詞',
    '飲食漸趨多樣化，依WHO建議母乳哺育可持續到2歲以上',
    '開始練習用湯匙進食，但仍常用手抓食物、容易灑落',
    '白天午睡漸漸從兩次減為一次，總睡眠約11–14小時',
  ], watch: [
    '走得更穩後容易攀爬家具，建議固定家具與電視防止傾倒',
    '樓梯口加裝安全門欄，避免獨自嘗試爬樓梯跌倒',
    '圓形或硬質食物（葡萄、堅果、熱狗）務必切小塊防噎',
    '收好鈕扣電池、硬幣等小物件，避免誤食風險',
  ]},
  { month: 17, tips: [
    '走路已經相當穩，開始嘗試蹲下撿東西後自己站起來',
    '能疊起2–3塊積木，塗鴉動作也更明顯、更有目的性',
    '開始能聽懂較少依賴手勢提示的簡單指令，但仍因人而異',
    '使用湯匙的動作更熟練，自己吃飯的意願也提高',
    '開始出現物權概念，可能會抱緊心愛的玩具不放手',
    '對圖畫書更感興趣，會主動跟著大人一起翻頁看圖',
  ], watch: [
    '開始會自行爬上沙發、椅子，留意攀爬跌落的風險',
    '抽屜、櫃子建議加裝安全鎖，避免夾傷或誤食清潔用品',
    '戶外活動增加，行走於馬路、停車場時務必牽好手',
    '地上的小物件、硬幣仍是常見噎塞風險，隨手清理環境',
  ]},
  { month: 18, tips: [
    '多數寶寶已能獨走自如，開始會走樓梯或小跑幾步',
    '會說3個以上單字，也開始模仿大人做家事的動作',
    '能用杯子喝水、湯匙吃飯，動作漸漸熟練不易灑落',
    '會指出有趣的東西想與你分享，情感表達更加豐富',
    '開始出現簡單的假扮遊戲，例如餵娃娃吃東西',
    '會配合穿衣動作，如伸手穿袖子、抬腳讓你穿褲子',
  ], watch: [
    '18個月是兒科例行健檢時間點，可趁機和醫師討論發展狀況',
    '走樓梯、攀爬能力提升，樓梯口與陽台仍需做好防護',
    '檢查家具、電視是否牢固定好，避免攀爬拉扯而翻倒',
    '藥品、清潔用品收到孩子拿不到的高處或上鎖收好',
    '汽車安全座椅建議儘量維持後向乘坐，依座椅限重調整',
  ]},
  { month: 19, tips: [
    '跑步動作更協調流暢，能一邊跑一邊繞過小障礙物',
    '詞彙量常明顯增加，正處於學說話的活躍期',
    '偶爾會把兩個字詞連在一起表達，例如「還要」「抱抱」',
    '自己拿杯子喝水、用湯匙吃飯更加獨立熟練',
    '開始有自己的主見，可能因表達受限出現短暫情緒起伏',
    '喜歡模仿大人的日常動作，如掃地、講電話、擦桌子',
  ], watch: [
    '活動範圍變大，靠近水池、浴缸等水域時務必全程看顧',
    '跑跳機會增加，留意尖角家具、濕滑地面等跌倒風險',
    '仍可能將小物放入口中，鈕扣電池、磁珠等要收好收高',
    '情緒起伏增加，留意推車、高腳椅上掙扎導致的跌落',
  ]},
  { month: 20, tips: [
    '詞彙量持續累積中，多數孩子已能說出「媽媽」「爸爸」以外的3個以上單字',
    '放手獨立行走已較穩定，開始嘗試自行爬上、爬下沙發或矮椅',
    '開始會模仿大人做家事的動作，例如假裝拿掃把掃地',
    '玩玩具的方式趨向簡單功能性，例如推玩具車、疊放物品',
    '用杯子喝水時仍常灑出來，這是正常練習過程，不用擔心',
    '一天飲食多落在3–4餐正餐、1–2次點心的節奏，依孩子食慾彈性調整',
  ], watch: [
    '孩子已能自行爬上爬下家具，跌倒風險提高，建議家具邊角加裝防護',
    '活動力與好奇心增加，矮櫃、抽屜內清潔劑、藥品建議移高或上鎖',
    '仍可能把小物品放入口中，留意地上鈕扣、電池等哽塞風險物',
    '書櫃、電視櫃等重家具若尚未固定，建議盡快用防傾倒扣具固定',
  ]},
  { month: 21, tips: [
    '語言快速進步中，開始會複述剛聽到的字詞，詞彙量持續擴充',
    '簡單假扮遊戲逐漸出現，例如假裝餵娃娃吃東西、哄娃娃睡覺',
    '手部動作更靈活，能疊起2–3塊積木、塗鴉線條',
    '自我意識增強，可能常說「不要」或出現抗拒指令的行為，屬常見發展階段',
    '睡眠多數已固定為白天一次午睡，加上夜間約11小時左右的睡眠',
    '進食時鼓勵孩子自己動手，跟著飢餓與飽足反應調整餵食步調',
  ], watch: [
    '自主意識與情緒起伏變明顯，硬碰硬效果有限，轉移注意力常更有效',
    '若家中樓梯尚未裝設安全門，此階段爬樓梯能力提升，需留意跌落風險',
    '整顆葡萄、堅果、爆米花、熱狗等仍屬哽塞高風險食物，建議切碎切小',
    '好奇心與活動範圍擴大，浴缸、水桶、泳池等水域務必全程有人看顧',
  ]},
  { month: 22, tips: [
    '開始出現把兩個字詞湊在一起的雛形，例如「還要」「抱抱」，邁向短句階段',
    '認知能力提升，會模仿簡單的排序、配對或分類遊戲',
    '手眼協調更好，疊積木、翻書頁等精細動作更加熟練',
    '大動作更穩健，開始嘗試小跑步、用腳踢球等活動',
    '使用湯匙進食的技巧持續進步，灑落狀況逐漸減少',
    '飲食建議涵蓋每天的肉、魚、蛋等動物性食物，搭配蔬菜水果維持均衡',
  ], watch: [
    '孩子開始能小跑、踢球，戶外活動時留意場地平坦與周邊車輛',
    '若考慮讓孩子接觸螢幕，建議選擇優質內容並全程陪同觀看，避免長時間獨自使用',
    '活動力與攀爬能力持續進步，門窗、陽台防護建議再次確認',
    '如廁訓練通常要到2歲後才逐漸準備好，此階段不需心急催促',
  ]},
  { month: 23, tips: [
    '逐漸接近24個月里程碑，開始能把兩個字組成小短句，例如「還要喝奶」',
    '開始會指認至少兩個身體部位，例如問「鼻子在哪裡」時能指出來',
    '除了揮手、用手指之外，開始會用點頭、送飛吻等手勢表達',
    '玩玩具方式更豐富，可能同時運用兩種玩具做出簡單情境',
    '開始會觀察大人表情，藉此判斷陌生情境該如何反應',
    '哺乳中的家庭可依世界衛生組織建議，持續哺乳到2歲以上並搭配固體食物',
  ], watch: [
    '孩子逐漸能踢球、走上矮階梯，留意樓梯、月台等高低落差環境的防護',
    '堅果、豆類等食物若要提供，建議壓碎或切碎，避免整顆造成哽塞',
    '即將滿2歲，多數診所會於2歲健兒門診安排發展篩檢，如有疑慮可提前與醫師討論',
    '情緒表達更豐富也可能更強烈，持續給予耐心與一致的引導方式',
  ]},
  { month: 24, tips: [
    '開始會跑，也能自己走上下幾階樓梯',
    '會用湯匙吃飯，動作愈來愈熟練',
    ...
    '活動範圍變大，居家防撞防跌要再加強',
    '開始吃固體食物，葡萄、堅果等圓硬食物仍要小心噎到',
    '汽座建議盡量維持後向安裝，直到達廠商身高體重上限',
    '好奇心旺盛，會轉旋鈕開關，藥品清潔劑要收到夠高夠緊處',
    '戲水泡澡即使會走，也要全程在伸手可及範圍內看顧',
  ]},
  { month: 30, tips: [
    '詞彙量明顯增加，約累積50個左右的詞彙',
    '開始把詞語組合著說，如「狗狗跑跑」',
    '喜歡假裝遊戲，像是假裝餵娃娃吃東西',
    '會轉門把、轉開瓶蓋，雙手更靈活協調',
    '能照著兩步驟指令做事，如「放下玩具、關門」',
    '開始會用「我」「你」，也認得至少一種顏色',
  ], watch: [
    '手部操作進步，門把瓶蓋擋不住好奇心，藥品化學品要收更高更緊',
    '會自己脫掉部分衣物，如廁訓練期間留意小物品誤吞',
    '開始有同伴互動，遊戲場留意碰撞推擠',
    '建議控制螢幕使用時間，臥室避免擺放電視等螢幕',
    '語言發展步調因人而異，稍慢不必過度焦慮',
  ]},
  { month: 36, tips: [
    '能與人來回對話至少兩輪',
    ...
    '理解力提升，開始聽懂「很燙不要碰」，但仍要大人在旁盯緊',
    '汽座達後向上限後可改前向，持續使用五點式安全帶',
    '社交活動變多，出遊上課仍要留意水域、停車場周邊安全',
    '騎滑步車、三輪車務必戴合適安全帽',
    '分離焦慮多數10分鐘內可平復，是常見的情緒發展',
  ]},
];
// GROWTH_MILESTONES is authored in ascending month order — exact match if this age has its
// own entry (true for every month 0–24), otherwise the nearest PRECEDING checkpoint (24, 30,
// or 36) rather than a future one, since a 26-month-old hasn't reached the 30-month content
// yet.
function findMilestoneData(ageMonths) {
  const exact = GROWTH_MILESTONES.find(g => g.month === ageMonths);
  if (exact) return exact;
  const preceding = GROWTH_MILESTONES.filter(g => g.month <= ageMonths);
  return preceding.length ? preceding[preceding.length - 1] : GROWTH_MILESTONES[0];
}
function monthsToTitle(totalMonths) {
  const years = Math.floor(totalMonths / 12), months = totalMonths % 12;
  if (years > 0) return years + '歲' + (months > 0 ? months + '個月' : '');
  return months + '個月';
}
function renderGrowthAdviceCard() {
  ...
  const data = findMilestoneData(ageMonths);
  ...
  const tipsHtml = list(data.tips);
  const watchHtml = data.watch && data.watch.length
    ? `<p style="font-size:12px;font-weight:700;color:#D2654A;margin:14px 0 4px;">⚠️ 該注意的事</p>${list(data.watch)}`
    : '';
  // Ages between real checkpoints (e.g. 26 months, between the 24- and 30-month checkpoints)
  // borrow the nearest earlier checkpoint's content — say so, rather than silently presenting
  // it as if it were written specifically for this exact age.
  const refNote = data.month !== ageMonths
    ? `<p style="font-size:10.5px;color:var(--text3);margin-top:2px;">目前沒有每個月都不同的細分內容，顯示最接近的 ${monthsToTitle(data.month)} 參考內容</p>`
    : '';
  ...
  return sCard(`🌱 ${monthsToTitle(ageMonths)}`, tipsHtml + watchHtml + refNote + pctNote);
}
...
function renderAuthCard() {
  ...
  if (u) {
    ...
    // fromCacheOnly can lag behind reality for a while (see firebase-sync.js's note on
    // metadata.fromCache) — records may already be syncing fine even while this still reads
    // true, so the wording stays soft/non-alarming instead of asserting the connection is
    // actually down.
    const statusLabel = Sync.pushFailures > 0
      ? `⚠️ 有 ${Sync.pushFailures} 筆變更沒上傳成功（${Sync.lastPushError}）`
      : Sync.fromCacheOnly && Sync.state === 'done' ? '🔄 同步狀態確認中…'
      : Sync.state === 'done' ? '✓ 即時同步中' : Sync.state === 'syncing' ? '連接中…' : (Sync.message || '—');
    const unhealthy = Sync.pushFailures > 0;
    return `
      ...
      <div style="display:flex;align-items:center;gap:8px;background:var(--card2);border-radius:12px;padding:10px 12px;">
        <div style="width:8px;height:8px;border-radius:50%;background:${unhealthy ? '#E5573D' : Sync.fromCacheOnly && Sync.state === 'done' ? '#C8965A' : Sync.state === 'done' ? '#5CB85C' : '#C8965A'};flex-shrink:0;"></div>
      ...
      ${Sync.fromCacheOnly && Sync.state === 'done' ? `<button onclick="A.forceResync()" style="width:100%;margin-top:10px;background:var(--card2);border:1.5px solid var(--inpBorder);border-radius:14px;padding:11px;font-size:13px;font-weight:700;color:var(--text2);">🔄 重新連線伺服器</button>
      <p style="font-size:10.5px;color:var(--text3);margin-top:8px;line-height:1.5;">同步狀態還在確認中，如果紀錄實際上有正常互通可以忽略；一直沒恢復的話可以點上面的按鈕重試。</p>` : ''}`;
  }
...
function diagnosticsText() {
  ...
  lines.push('fromCacheOnly: ' + Sync.fromCacheOnly + (Sync.fromCacheOnly ? '  ← 尚未確認連線（可能只是還沒收到能證明連線的新資料，不一定代表真的斷線）' : ''));
